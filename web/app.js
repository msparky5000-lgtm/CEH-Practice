...
function normaliseQuestion(q, chapter) {
  return {
    ...
    answers: (q.answers || []).map((answer, index) => {
      ...
      return match ? { letter: match[1], text: match[2], full: answer } : {
...
function startExam() {
  ...
  if (byChapter.some((list) => list.length < 7)) {
  ...
  const exam = [];
  byChapter.forEach((list, index) => exam.push(...shuffle(list).slice(0, bonusIndexes.includes(index) ? 7 : 6)));
...
function finishQuiz() {
  ...
  $('resultScore').textContent = `Score: ${correctCount}/${total}`;
  ...
  $('incorrectStat').textContent = `Incorrect: ${total - correctCount}`;
  ...
  Object.entries(analytics).sort().forEach(([chapter, stats]) => {
    ...
    row.textContent = `${chapter}: ${stats.correct}/${stats.total} answered (${pct.toFixed(0)}%)`;
...
function reviewIncorrect() {
  ...
  list.innerHTML = '';
  // Only checked answers from this session can enter state.results.
  const wrong = state.results.filter((r) => r && r.is_correct === false);

  if (!wrong.length) {
    list.innerHTML = '<p class="perfect">🎉 No answered questions were incorrect.</p>';
  } else {
